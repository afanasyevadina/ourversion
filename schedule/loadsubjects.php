...
foreach($items as $item) { 
	if($item['sem'.$_REQUEST['sem']]>0) {
		...
		if(in_array($item['general_id'], $warnings)||(!$warning&&$item['theory']>0)||!in_array($item['general_id'], $gs)) {
			?>
			<li data-teacher="<?=$item['teacher_id']?>" data-id="<?=$item['item_id']?>" style="background-color:<?=$color?>" class="sub_item <?=$divide?>"><?=$item['subject_name'].' '.$subgroup?> <i><?=$item['teacher_name']?></i><span><?=$gone?>/<?=$item['sem'.$_REQUEST['sem']]?></span></li>
	<?php }
	}
} ?>
